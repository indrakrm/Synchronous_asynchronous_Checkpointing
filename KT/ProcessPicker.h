...
{
public:
	int totalNodes,stop=0,wait=0,id=0;
	...
	void picker()
	{
		...
		while(!this->stop)
		{
			...
			if(i>4)
				continue;
			int node_id=1+rand()%(totalNodes-1);
			sleep(1);
			sendMsg(node_id,"0,initiate checkpoint,");
			auto start = high_resolution_clock::now();
			wait=1;
			if(this->stop)
				break;
			cout<<"intiate checkpoint to "<<node_id<<endl;
			while(wait);
			auto stop = high_resolution_clock::now(); 
			auto duration = duration_cast<microseconds>(stop - start);
			cout<<"Checkpoint"<<i+1<<" completed and it took "<<duration.count()<<" ms."<<endl;
			sleep(5);
			node_id=1+rand()%(totalNodes-1);
			sendMsg(node_id,"0,initiate recovery,");
			wait=1;
		...
		auto d = duration_cast<microseconds>(s - st);
		cout<<"Completeion time = "<<d.count()<<" ms."<<endl;
	...
	void receive()  //socket implementation to receive messages
	{
		while(!this->stop)
		{
			...
			if(strcmp(message,"recovery success")==0 || strcmp(message,"checkpoint success")==0 || strcmp(message,"recovery failure")==0 || strcmp(message,"checkpoint failure")==0)
			{
				wait=0;
			}
		}
